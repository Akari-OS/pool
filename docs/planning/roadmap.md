...
- **Workspace モデル** で目的別に隔離、明示的に横断可能
- **AGPL-3.0** でソース公開を担保
...
═══ Phase 2: Workspace 隔離 + 横断検索（完了）══════════════════════

  Pool を workspace マネージャに    ██████████ 100%  ✅
  Workspace 構造体 + CRUD           ██████████ 100%  ✅
  workspace.toml + config.toml      ██████████ 100%  ✅
  FTS5 検索 + --cross               ██████████ 100%  ✅
  パストラバーサル対策              ██████████ 100%  ✅
...
### Phase 2: Workspace 隔離 + 横断検索 ✅ 完了 (2026-04-08)

**目的**: 目的別に作業空間を隔離し、明示的に横断検索できるようにする。

**完了基準**:
- `workspace create / list / use / info / delete` が動く
- カレント workspace が `--workspace` > env > config > "default" で解決される
- パストラバーサル攻撃を拒否する
- `search --cross` で全 workspace 横断できる
...
- MCP リソース: `pool://item/{id}` / `pool://workspace/{name}` / `m2c://item/{id}/context`
...
- 動画系 workspace（例: `wedding-2026`）での実運用テスト
