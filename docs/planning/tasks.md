...
## 現在地 — Phase 6 完了（2026-05-08）

Phase 0〜6 を完走。34 MCP ツール稼働中。次は **Phase 7（AKARI Video 統合）**。
...
| Library 隔離 + `library.toml` | ✅ | 2 | `906f972` |
...
| Linter::check_inconsistency（タグ揺れ検出） | ✅ | 4.5 | — |
| Linter::check_connection_gap（関係抜け補完） | ✅ | 4.5 | — |
| Relation CLI（add / list / rm） | ✅ | 4.5 | — |
| `lint show` — 保存済み findings 閲覧 | ✅ | 4.5 | — |
| Filed-back ループ（notes/ 監視 → Pool 反映） | ✅ | 5 | — |
| `[[backlink]]` → relation 自動作成 | ✅ | 5 | — |
| LLM モデルプリセット 6 種 + 3 層設定マージ | ✅ | 5 | — |
| pool-mcp crate（rmcp）stdio + Streamable HTTP | ✅ | 6 | — |
| 34 MCP ツール + pool://library/ リソース | ✅ | 6 | — |
| 会話履歴永続化 + Pool Browser MVP | ✅ | 6 | — |
...
- [x] **T3.5** `Library::analyze_item` + `add_item_and_analyze`（パーシャル成功対応） (FR-04-06)
...
- [x] **T4.1** `wiki.rs::compile_item_to_markdown(item, library_name, related)` 本実装 (FR-06-01)
...
- [x] **T4.5.1** `Linter::check_inconsistency` 実装 (FR-08-04)
  - [x] 編集距離ベースのタグ揺れ検出（レーベンシュタイン、n-gram）
  - [x] LLM による正規化提案（OpenRouter 経由）
  - [x] `Severity::Warn` で finding 生成
  - [x] 単体テスト（タグ集合を入力、期待 finding を検証）
- [x] **T4.5.2** `Linter::check_connection_gap` 実装 (FR-08-05)
  - [x] `pool_relations` からグラフを構築
  - [x] A→B / B→C はあるが A→C が無いケースを探索
  - [x] `Severity::Info` で finding 生成
  - [x] 推移探索の深さ上限を設定（爆発防止）
- [x] **T4.5.3** Relation CLI 実装 — `crates/pool-cli/src/main.rs` (FR-07-06, FR-09-11)
  - [x] `akari-pool relation add <from> <to> --type <type> [--strength <f>] [--note <text>]`
  - [x] `akari-pool relation list [--item <id>]`
  - [x] `akari-pool relation rm <relation_id> --confirm`
- [x] **T4.5.4** `akari-pool lint show [--severity <s>]` 実装 (FR-08-07, FR-09-12)
  - [x] DB 保存済み（resolved_at IS NULL）の findings を列挙
  - [x] severity フィルタ
  - [x] 件数サマリ表示
- [ ] **T4.5.5** `docs/design/` に `linter-extended.md` を追加（4 種の検出ロジックと閾値）
- [x] **T4.5.6** Review ラウンド（2〜3 並列）: アルゴリズム計算量 / LLM プロンプト / エッジケース / CLI UX
- [x] **T4.5.7** 実機スモーク: 10〜20 アイテム + 数件 relation で 4 種の finding を目視確認
- [x] **T4.5.8** `cargo test --workspace` + `cargo install --path crates/pool-cli --force`
- [x] **T4.5.9** commit `[機能追加] Phase 4.5 — Linter 拡張 + Relation CLI`
- [x] **AC**: 4 種全ての Lint チェッカーが動作、Relation を CLI から追加・一覧・削除できる、`lint show` で保存済み findings を severity フィルタ付きで閲覧できる
...
- [x] **T5.2** ファイル変更監視（`notify` crate）or `akari-pool sync` コマンド (FR-09-14)
- [x] **T5.3** frontmatter 差分抽出（ai_summary / ai_tags の編集を検知）
- [x] **T5.4** `[[item-id]]` 記法 → relation 自動作成 (FR-07-07)
- [x] **T5.5** モデルプリセット UI（CLI フラグ） (FR-05-08)
  - [x] 💰節約 / ⚖️バランス / ⚡パフォーマンス / 🔒オフライン / 🧪ハイブリッド / 🎓高品質
- [x] **T5.6** 3 層設定（global → library.toml → セッション/CLI） (FR-05-09)
- [x] **AC**: Obsidian で編集 → `sync` で Pool 反映、`[[item-id]]` で relation 作成、6 プリセットが CLI から選べる
...
- [x] **T6.1** `rmcp` crate 最新バージョン確認 + `pool-mcp` 実装 (FR-10-01)
- [x] **T6.2** stdio + Streamable HTTP 両対応 (FR-10-02)
- [x] **T6.3** MCP ツール: `pool_ls / pool_cat / pool_grep / pool_search / pool_add / pool_lint / pool_compile_notes` (FR-10-03)
- [x] **T6.4** MCP リソース: `pool://item/{id}` / `pool://library/{name}` / `m2c://item/{id}/context` (FR-10-04)
- [x] **T6.5** library 引数はセッション初期化時に固定 (FR-10-05)
- [x] **T6.6** `akari-pool mcp-serve [--http] [--port]` (FR-09-13)
- [x] **T6.7** AKARI Video の既存 MCP と整合性確認 (FR-10-06)
- [x] **T6.8** 実機テスト: Claude Code から接続し `pool_search` を呼ぶ（34 ツール稼働確認）
- [x] **AC**: Claude Code / Cursor から MCP で AkariPool に接続し、ツールを呼べる
