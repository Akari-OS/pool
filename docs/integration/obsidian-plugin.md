...
→ AKARI Video の既存 FFmpeg ラッパで対応可能。`libraries/{lib}/files/{uuid}/proxy.mp4` のような命名で並置する。
...
| 2 | **Library セレクタ** | 左サイドバーで library 切替 | 1日 |
...
├── views/
│   ├── pool-sidebar.ts        — 左サイドバー（library セレクタ）
...
│   └── library-detector.ts    — Vault → library 検出
├── commands/
...
設定変更は即座に AkariPool 側の `library.toml` に書き込まれる。
...
| 複数 Vault 間の library 衝突 | Vault パスから library を一意特定する規約 |
...
- [`../design/library-layer.md`](../design/library-layer.md) — Library モデル
...
- [ ] **複数 library の同時表示**: 1つの Obsidian ウィンドウで複数 library を見たい場合の UI
