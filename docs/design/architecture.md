...
> **バージョン**: v0.1 Draft
> **最終更新**: 2026-04-08
> **ステータス**: Phase 1 完了 (2026-04-08) → 次は Phase 2 (Workspace 隔離)
...
| 3 | **Workspace 隔離** | デフォルトは現在の workspace 内のみ。明示で横断 |
...
│                       コアロジック層                                │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐            │
│  │ pool-core    │  │ pool-workspace│  │ pool-lint    │            │
...
### 4-2. なぜ workspace ごとに pool.db を分けるか
- **隔離が物理的に強制される**（誤って横断検索しない）
- **バックアップ/移動が単純**（フォルダごと持っていける）
- **削除が安全**（workspace 削除 = フォルダ削除のみ）
- **横断検索は呼び出し側で複数 DB を open する**（Phase 2 で実装）

詳細: [`workspace-layer.md`](./workspace-layer.md)
...
CREATE TABLE pool_relations (
    ...
    relation_type TEXT NOT NULL,          -- derived/related/variant/series/style_used/remix/shared_tag/cross_workspace
    strength REAL NOT NULL,               -- 0.0-1.0
    created_by TEXT,                      -- ai/user/lint
    reason TEXT,                          -- 推論根拠
    target_workspace TEXT,                -- NULL=同workspace, 値あり=横断リンク
    created_at TEXT NOT NULL,
...
| **pool-workspace** | Workspace 隔離、横断検索、共有レイヤー | pool-core |
| **pool-lint** | 4種の健全性チェック、Linter trait | pool-core |
| **pool-mcp** | MCP server (rmcp) - tools/resources/subscribe | pool-core, pool-workspace, pool-lint, rmcp |
...
pub trait Linter: Send + Sync {
    fn name(&self) -> &'static str;
    async fn check(&self, ws: &Workspace) -> Vec<Finding>;
}
...
ユーザー or AI が pool add
       ↓
[1] ファイルを workspaces/{ws}/files/{uuid}/ にコピー
       ↓
...
| レイヤー | 境界 | 強制方法 |
|---|---|---|
| Workspace 間 | 物理的に DB ファイル別 | `Workspace::open` でパス検証 |
| File system 越境 | `~/.akari-pool/` 配下のみ書き込み | path canonicalize、startsWith 禁止 |
| MCP クライアント | Roots ベース、初期化時に workspace 固定 | rmcp の roots 機能 |
...
| **OpenViking** | viking:// URI の階層モデル。AkariPool の Workspace モデルが類似 |
...
| **2** | pool-workspace | Workspace 隔離、横断検索、共有レイヤー | 🟡 **次** |
...
- [ ] Workspace 間の関係グラフ表現（cross_workspace relation の DB スキーマ）
