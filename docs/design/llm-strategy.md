...
| 7 | **3層設定** | グローバル → workspace → セッション の優先順位 |
| 8 | **コスト透明性** | リアルタイムでトークン消費・推定料金を表示 |
| 9 | **プライバシーファースト** | センシティブな workspace は「外部 API 禁止」フラグで保護（このときローカル必須） |
...
外部 API 完全ブロックの workspace 用。ユーザーが Ollama + Qwen3 + Qwen2.5-VL + whisper.cpp + fastembed を全部入れた場合のみ動く。
...
### Layer 2: Workspace 設定

`~/.akari-pool/workspaces/wedding-2026/workspace.toml`:

```toml
[workspace.llm]
# この workspace 専用のオーバーライド
default_model = "qwen3:14b"  # 結婚式は質を優先

[workspace.llm.privacy]
# プライバシー: 外部 API 禁止
external_api_allowed = false   # ← これで OpenRouter 系全部ブロック
local_only = true

[workspace.llm.cascade]
...
- 違反検出時はエラー + ログ
- センシティブな workspace（医療・法務・個人情報）に必須
...
# 4. プール作成して動かす
akari-pool workspace create my-first-pool
akari-pool add ./video.mp4 --workspace my-first-pool
```
...
- **モデルマーケットプレイス**: ユーザー間で「俺の workspace 設定」を共有
...
- [`workspace-layer.md`](./workspace-layer.md) — Workspace の privacy 設定
