...
新: ~/.akari-pool/libraries/akari-video-default/pool.db
```
...
| `pool_items.file_path` | `pool_items.file_path` | パス書き換え（library 相対に） |
...
| `pool_relations.*` | `pool_relations.*` | ほぼ直接（`target_library` は NULL） |
...
pub fn migrate_from_akari_video_v5(
    ...
    // 4. raw ファイルを library 配下にコピー（ハードリンク or シンボリックリンク）
...
impl PoolClient {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self { pool, default_library: ws_name })
    }
...
- [ ] **複数 AKARI Video プロジェクト**を別 library にすべきか、1つの library に集約すべきか
  - 案 A: 1 AKARI プロジェクト = 1 library（直接対応）
  - 案 B: library = ユーザー意図、プロジェクトはその中の概念
